...
<script>
    ...
    function validation(){
        if (document.getElementById('exist_error_message').innerHTML == "Format Name exists"){
            alert("Format Name Exist.Pls Enter Different Unique Format Name...!");
            return false;
        }
    }
</script>
        ...
        <h4>
            ...
            <ul class="dropdown-menu" role="menu">
                {foreach name=list item=item key=key from=$print_copy_list}
                    <li><a href='#' data-toggle="modal" data-target="#pdf_download_modal" onclick="add_sop_download_history('{$regobj->sop_object_id}','{$regobj->format_object_id}','index.php?module=file&action=view_format&sop_object_id={$regobj->sop_object_id}&type={$item.object_id}&format_id={$regobj->format_object_id}')">{$item.copy_type}</a></li>
                {/foreach}
            </ul>
            ...
            <span class="font-semibold"> {$regobj->format_name} </span><span class="vd_soft-grey"> - Editing Mode </span>
        </h4>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header vd_bg-blue vd_white">
                        ...
                        <h4 class="modal-title" id="myModalLabel">Edit Format Name</h4>
                    </div>
                    <form name="edit_format_name_form" method="post" action="{$smarty.server.REQUEST_URI}" class="form-horizontal" role="form" id="edit_format_name_form" autocomplete="off">
                        <div class="modal-body">
                            <div class="form-group">
                                <div class="col-md-12">
                                    <label class="control-label  col-sm-2">{attribute_name module="sop" attribute="sop_format_name"} <span class="vd_red">*</span></label>
                                    <div id="first-name-input-wrapper"  class="controls col-sm-10">
                                        <input type="text" placeholder="Min 3 - Max 80" class="width-100 required" name="format_name" id="format_name" value="{$regobj->format_name}" maxlength="80" required title="Enter Valid Format Name" onkeyup="format_exists('{$regobj->sop_object_id}')">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                        ...
                        <div class="modal-footer background-login">
                            ...
                            <button type="submit" class="btn vd_btn vd_bg-green" name="edit_format_name" onClick="return validation();">Save changes</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal fade" id="enable_disable_Modal" tabindex="-1" role="dialog" aria-labelledby="enable_disable_Modal_ModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header vd_bg-blue vd_white">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i></button>
                        <h4 class="modal-title" id="enable_disable_ModalLabel">Enable / Disable Format Name</h4>
                    </div>
                    <form name="enable_disable_format_name_form" method="post" action="{$smarty.server.REQUEST_URI}" class="form-horizontal" role="form" id="add_sop_form-form" autocomplete="off">
                        <div class="modal-body">
                            <div class="form-group">
                                <div class="col-md-12">
                                    ...
                                    <div id="first-name-input-wrapper"  class="controls col-sm-10">
                                        <select class="width-60" name="enable_disable" id="enable_disable">
                                            ...
                                            <option value="Enabled" {if $sop_format_status eq "Enabled"}selected{/if}>Enable</option>
                                            <option value="Disabled" {if $sop_format_status eq "Disabled"}selected{/if}>Disable</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="modal-footer background-login">
                            <button type="button" class="btn vd_btn vd_bg-grey" data-dismiss="modal">Close</button>
                            <button type="submit" class="btn vd_btn vd_bg-green" name="edit_format_status" onClick="return status_validation();">Save changes</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form name="orientation_form" method="post" action="{$smarty.server.REQUEST_URI}" class="form-horizontal" role="form" id="orientation_form" autocomplete="off">
                            ...
                            <div class="form-group">
                                <div class="col-md-12">
                                    ...
                                    <div id="first-name-input-wrapper"  class="controls col-sm-9">
                                        <select class="width-60" name="format_ori" id="format_ori" required title="Select Valid Orientation">
                        ...
                        <div class="modal-footer background-login">
                            <button type="button" class="btn vd_btn vd_bg-grey" data-dismiss="modal">Close</button>
                            <button type="submit" class="btn vd_btn vd_bg-green" name="edit_format_orientation">Save changes</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="panel light-widget widget">
                    ...
                    <div class="panel-body">
                        <div class="col-md-12">
                            ...
                            {if $sop_format_status eq "Disabled"}
                                <div class="alert alert-danger alert-dismissable alert-condensed">
                                    <i class="fa fa-exclamation-triangle append-icon"></i><strong>Oh snap!</strong> <a href="#" class="alert-link">Format Disbled.</a>
                                </div>
                            {/if}
                            <form name="edit_format_p" method="post" action="{$smarty.server.REQUEST_URI}" class="form-horizontal" role="form" id="edit_format_p-form" autocomplete="off" >
                                {if $sop_format_status neq "Disabled" and $a4p}
                                    <div class="row" style="display: {$a4p}">
                                        <div class="container">
                                            <textarea name="editor_details_p" id="editor_detail_p" style="resize: none">{$sop_format_editor_details}</textarea>
                                        </div>
                                    </div>
                                {/if}
                            </form>
                            <form name="edit_format_l" method="post" action="{$smarty.server.REQUEST_URI}" class="form-horizontal" role="form" id="edit_format_l-form" autocomplete="off" >
                                {if $sop_format_status neq "Disabled" and $a4l}
                                    <div class="row" style="display: {$a4l}">
                                        <div class="container">
                                            <textarea name="editor_details_l" id="editor_details_l" style="resize: none">{$sop_format_editor_details}</textarea>
                                        </div>
                                    </div>
                                {/if}
                            </form>
...
{literal}
    ...
    <script>
        function format_exists(sop_object_id){
            document.getElementById('format_name').value=document.getElementById('format_name').value.toUpperCase();
            var format_name = document.getElementById('format_name').value;
            x_format_name_exist(format_name,sop_object_id,check_handle); //Call SAJAX JavaScript... Pass Department id as Value and name of the function which is going to handle the reply 
        }
        function check_handle(result) {
            if (result=="true") {
                document.getElementById('exist_error_message').innerHTML = "Format Name exists";
