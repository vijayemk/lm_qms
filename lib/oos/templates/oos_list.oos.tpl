        ...
        <div class="panel-group" id="accordion">
            <div class="panel panel-default">
                <div class="panel-heading vd_bg-dark-green">
                    ...
                    <span class="label label-danger mgl-10">{$oosLists|@count}</span>
                </div>
                <div id="collapsedmslist" class="panel-collapse collapse in">
                    <div class="panel-body">
                        <div class="panel widget light-widget">
                            <form name="oos_list-form" method="post" action="{$smarty.server.REQUEST_URI}" class="form-horizontal" role="form" id="oos_list-form" autocomplete="off">
                                {if !empty($oosLists)}
                                    <table class="table table-bordered table-striped generate_datatable" title="OOS List" data-sort_column=1 data-whitespace="nowrap" data-show_rows="15">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>{attribute_name module="oos" attribute="oos_no"}</th>
                                                <th>{attribute_name module="admin" attribute="plant_name"}</th>
                                                <th>{attribute_name module="oos" attribute="department"}</th>
                                                <th>{attribute_name module="oos" attribute="initiator"}</th>
                                                <th>{attribute_name module="oos" attribute="date_of_occurrence"}</th>
                                                <th>{attribute_name module="oos" attribute="date_of_reporting"}</th>
                                                <th>{attribute_name module="oos" attribute="target_date"}</th>
                                                <th>{attribute_name module="oos" attribute="completion_date"}</th>
                                                {* <th>{attribute_name module="oos" attribute="approval_status"}</th> *}
                                                <th>{attribute_name module="oos" attribute="wf_status"}</th>
                                                <th>{attribute_name module="oos" attribute="status"}</th>

                                            </tr>
                                        </thead>
                                        <tbody>
                                            {foreach name=list item=item key=key from=$oosLists} 
                                                <tr>
                                                    <td></td>
                                                    <td>{$item.doc_link}</td>
