...
/**
 * Project:     Logicmind
 * File:        view_mformat_file_p.sop.php
...
if ($sop_process->is_sop_pdf_page_expired($sop_object_id, $format_object_id, $access_id) == true) {
...
if ($sop_master->get("sop_object_id", $sop_object_id)) {
    ...
    $mpdf->SetHTMLHeader('
        ...
        <table width="100%" style="border: 0.5px solid #000000;border-collapse: collapse;">
            <tr>
                ...
                <td width="17%" style="padding:8px;font-size: 13px;font-family: muli;font-normal: bold;color:#000000;text-align:center;border:1px solid #000000;">Format No</td>
                <td width="23%" style="padding:8px;font-size: 13px;font-family: muli;font-weight: bold;color:#000000;text-align:center;border:1px solid #000000;">' . $sop_process->get_sop_format_no($format_master->format_object_id) . '</td>
            </tr>
            <tr>
            ...
            <tr>
                <td rowspan="2" width="50%" style="padding:5px;font-size: 13px;font-family: muli;font-weight: bold;color:#000000;text-align:center;border:1px solid #000000;">' . $format_master->format_name . '</td>
    ...
    <table width="100%" style="margin: 5px 0px 0px 0px;padding:1px;">
        <tr>
            <td width="15%" style="font-size: 12px;font-family: arial;font-weight: bold;">' . $sop_no . '</td>
    ...
    <div style="width: 100%; text-align:center;font-size: 7px;font-family: arial;padding:2px;"> <span >CONFIDENTIAL : Do not copy, print or transmit in electronic and any other format without the permission of the authorities of ' . $sop_plant . '</span><span ></span><br>
    </div>
    ');
    $mpdf->AddPage();
    //$mpdf->ln();
    $sop_format_details = new DB_Public_lm_sop_format_details();
    $sop_format_details->get("format_object_id", $format_master->format_object_id);
    if ($format_master->status == "Enabled") {
        $format_details = $sop_format_details->format_content;
    } else {
...
ENDOFSTRING;
        $format_details = $error_message;
    }
    if (!empty($format_details)) {
        $mpdf->WriteHTML($format_details);
    }
    $mpdf->Output("$sop_no-Format.pdf", 'I');
} else {
    $error_handler->raiseError("INVALID REQUEST");
}
?>
