...
$exit_user_id = (isset($_REQUEST['exit_user_id'])) ? $_REQUEST['exit_user_id'] : null;
$user_obj = new DB_Public_users();
if ($user_obj->get("user_id", $exit_user_id)) {
    if ($user_obj->account_status == "disable") {
        $exit_obj = new DB_Public_user_exit();
        $exit_obj->get("user_id", $exit_user_id);
        $exit_object_id = $exit_obj->object_id;

        $user_name = $user_obj->user_name;
        $full_name = $user_obj->full_name;
        $emp_id = $user_obj->emp_id;
        $user_department = getDeptName($exit_user_id);
        ...
        $requested_by = getFullName($exit_obj->creator);
        $requested_date = get_modified_date_time_format($exit_obj->created_time);
        $request_no = $exit_obj->request_no;
        ...
        if (!empty($sop_process->get_creater_id_digital_sign($exit_object_id))) {
            $creator = $sop_process->get_creater_id_digital_sign($exit_object_id);
            $creator_name = getFullName($creator);
            $creator_dept = getDeptName($creator);
            $creator_designation = getDesignationByUserId($creator);
            $created_time = $sop_process->get_created_time_digital_sign($exit_object_id);
        }if (!empty($sop_process->get_reviewer_id_digital_sign($exit_object_id))) {
            $reviewer = $sop_process->get_reviewer_id_digital_sign($exit_object_id);
            $reviewer_name = getFullName($reviewer);
            $reviewer_dept = getDeptName($reviewer);
            $reviewer_designation = getDesignationByUserId($reviewer);
            $reviewed_time = $sop_process->get_reviewed_time_digital_sign($exit_object_id);
        }if (!empty($sop_process->get_approver_id_digital_sign($exit_object_id))) {
            $approver = $sop_process->get_approver_id_digital_sign($exit_object_id);
            $approver_name = getFullName($approver);
            $approver_dept = getDeptName($approver);
            $approver_designation = getDesignationByUserId($approver);
            $approved_time = $sop_process->get_approved_time_digital_sign($exit_object_id);
        }if (!empty($sop_process->get_exit_copy_user_id_digital_sign($exit_object_id))) {
            $deactivator = $sop_process->get_exit_copy_user_id_digital_sign($exit_object_id);
            $deactivator_name = getFullName($deactivator);
            $deactivator_dept = getDeptName($deactivator);
            $deactivator_designation = getDesignationByUserId($deactivator);
            $deactivated_time = $sop_process->get_exit_copy_deactivated_time_digital_sign($exit_object_id);
        }
    } else {
    ...
    class USER_EXIT_PDF extends TCPDF {
...
//            /$image_file = _URL_ . "img/custom_logicmind_img/logo.png";
            ...
            $this->Cell(0, 0, 'USER EXIT FORM', 0, 1, 'C', 0, '', 0, false, 'M', 'M');
        }
    ...
    $pdf = new USER_EXIT_PDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
    ...
    $pdf->SetSubject('User Exit Details');
    $pdf->SetKeywords('User Exit Details');
    ...
    $pdf->SetFont('calibri', '', 10, true);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->Cell(25, 5, 'Request No. :', 0, false, 'R', 0, '', 0, false, 'M', 'M');
    $pdf->SetFont('calibri', 'B', 10, true);
    $pdf->SetTextColor(0, 0, 0);
    ...
    if (empty($creator)) {
        ...
        $pdf->SetFont('calibri', 'B', 10, true);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->Cell(180, 5, "Account Deactivated manually by Admin", 0, false, 'C', 0, '', 0, false, 'M', 'M');
    } else {
        ...
        if (!empty($creator)) {
            ...
            $valid_activator_digital_sign_info = "Digitally Signed Information \nRequest No : $request_no \nDeActivated By : $deactivator_name \nDesigntation : $deactivator_designation \nDepartment : $deactivator_dept \nDate : $activated_time";
        }
        ...
        if (!empty($creator_name)) {
            $pdf->Image(_URL_ . 'images/sop_images/valid.jpg', $pdf->GetX() + 25, $pdf->GetY() - 2.2, 4, 4, 'JPG', $valid_creator_digital_sign_info, '', true, 200, '', false, false, 0, false, false, false);
        } else {
            $pdf->Image(_URL_ . 'images/sop_images/invalid.jpg', $pdf->GetX() + 25, $pdf->GetY() - 2.2, 4, 4, 'JPG', $invalid_digital_sign_info, '', true, 150, '', false, false, 0, false, false, false);
        }
        ...
        if (!empty($reviewer_name)) {
            $pdf->Image(_URL_ . 'images/sop_images/valid.jpg', $pdf->GetX() + 25, $pdf->GetY() - 2.2, 4, 4, 'JPG', $valid_reviewer_digital_sign_info, '', true, 150, '', false, false, 0, false, false, false);
        } else {
            $pdf->Image(_URL_ . 'images/sop_images/invalid.jpg', $pdf->GetX() + 25, $pdf->GetY() - 2.2, 4, 4, 'JPG', $invalid_digital_sign_info, '', true, 150, '', false, false, 0, false, false, false);
        }
        ...
        if (!empty($approver_name)) {
            $pdf->Image(_URL_ . 'images/sop_images/valid.jpg', $pdf->GetX() + 25, $pdf->GetY() - 2.2, 4, 4, 'JPG', $valid_aprover_digital_sign_info, '', true, 150, '', false, false, 0, false, false, false);
        } else {
            $pdf->Image(_URL_ . 'images/sop_images/invalid.jpg', $pdf->GetX() + 25, $pdf->GetY() - 2.2, 4, 4, 'JPG', $invalid_digital_sign_info, '', true, 150, '', false, false, 0, false, false, false);
        }
        ...
        $pdf->SetFont('calibri', 'B', 8);
        $pdf->SetTextColor(0, 0, 128);
        if (!empty($deactivator_name)) {
            $pdf->Image(_URL_ . 'images/sop_images/valid.jpg', $pdf->GetX() + 25, $pdf->GetY() - 2.2, 4, 4, 'JPG', $valid_activator_digital_sign_info, '', true, 150, '', false, false, 0, false, false, false);
        } else {
            $pdf->Image(_URL_ . 'images/sop_images/invalid.jpg', $pdf->GetX() + 25, $pdf->GetY() - 2.2, 4, 4, 'JPG', $invalid_digital_sign_info, '', true, 150, '', false, false, 0, false, false, false);
        }
        $pdf->Cell(33, 5, "DeActivated By ", 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $pdf->SetFont('calibri', '', 8);
        $pdf->SetTextColor(0, 0, 128);
        if (!empty($deactivator)) {
            $pdf->Cell(72, 5, $deactivator_name, 0, false, 'L', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(30, 5, $deactivator_designation, 0, false, 'L', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(22, 5, $deactivator_dept, 0, false, 'L', 0, '', 0, false, 'M', 'M');
            $pdf->Cell(33, 5, $deactivated_time, 0, false, 'L', 0, '', 0, false, 'M', 'M');
        }
        $pdf->SetLineStyle(array('width' => 5, 'cap' => 'round', 'join' => 'round', 'dash' => '2'));
        $pdf->Line(15, $pdf->GetY() + 5, 200, $pdf->GetY() + 5, $style = array('width' => 0.05, 'color' => array(60, 179, 230)));
    }
