    ...
    <div class="panel-group" id="accordion">
        <div class="panel panel-default">
            <div class="panel-heading vd_bg-dark-green">
                <h4 class="panel-title"> <a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo">Add Exam Attempt Limit </a> </h4>
            </div>
                        ...
                        <div class="panel-body">
                            <div class="modal-wide-width">
                                <div class="modal-content">
                                    ...
                                    <div class="panel-body">    
                                        <form name="add_exam_attempt_limit-form" method="post" action="{$smarty.server.REQUEST_URI}" class="form-horizontal" role="form" id="add_exam_attempt_limit-form" autocomplete="off">
                                            ...
                                            <div class="form-group">
                                                <div class="col-md-3">
                                                    <label class="control-label">{attribute_name module="admin" attribute="current_attempt_limit"} <span class="vd_red">*</span></label>
                                                    <div class="controls">
                                                        <input type="text"  class="required" value="{$dm_exam_attempt_limit}" readonly>
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <label class="control-label">{attribute_name module="admin" attribute="change_to"} <span class="vd_red">*</span></label>
                                                    <div class="controls">
                                                        <select class="required" required name="uattempt_limit" id="uattempt_limit" title="Select Limit">
                                                            {foreach name=list item=item key=key from=$limit_range}
                                                                <option value="{$item}"{if $item eq $dm_exam_attempt_limit}selected{/if}>{$item}</option>
                                                            {/foreach}
                                                        </select>                                        
                                                    </div>
                                                </div>
                        ...
                        <div class="col-md-12">
                            <div class="panel widget">
                                <div class="panel-body table-responsive">
                                    {if isset($dm_exam_limit_remarks)}
                                        <table class="table table-bordered table-striped generate_datatable" title="Online exam attrmpt limit history list">
                                            <thead>
                                                <tr>
                                                    <td>{attribute_name module="admin" attribute="sl_no"}</td>
                                                    <th>{attribute_name module="admin" attribute="changed_from"} Time(s)</th>
                                                    <th>{attribute_name module="admin" attribute="changed_to"} Time(s)</th>
                                            ...
                                            <tbody>
                                                {foreach name=list item=item key=key from=$dm_exam_limit_remarks} 
                                                    <tr >
                                                        <td></td>
...
{literal}
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            "use strict";
            // Add exam attempt limit
            var form_submit = $('#add_exam_attempt_limit-form');
